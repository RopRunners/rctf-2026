...
MAX_THREADS = int(os.getenv("MAX_THREADS", default=2 * (os.cpu_count() or 1)))
...
# The game's address space. A service that pins a docker network inside it
# blackholes that team's own players from *inside* their box: the container
# route wins over the WireGuard route, so the checker (arriving from the
# router) still passes while the team cannot reach their own service. It is the
# hardest possible thing to debug mid-round, and the symptom points at the
# network rather than at a compose file.
#
# rctf-vulnbox/build.sh used to refuse to build on this, and that build is gone
# (rctf-deploy/DESIGN.md D8). rctf-deploy's terraform still scans for it but a
# `check` block only warns, and it cannot see a change made after `apply`. This
# is the hard gate, in the repo that owns the services and on every CI run.
GAME_NETWORK = ipaddress.ip_network("10.0.0.0/8")
...
def colored_log(*messages, color: ColorType = ColorType.INFO):
    ...
    # Logs go to stderr so that stdout stays machine-readable: "dump_tasks"
    # is meant to be piped straight into a ForcAD config.
    print(
        f"{color}{color.name} [{current_thread().name} {ts}]{ColorType.ENDC}",
        *messages,
        file=sys.stderr,
    )
...
class Checker(BaseValidator):
    ...
    def _run_command(self, command: List[str], env=None) -> Tuple[str, str]:
        ...
        # Copy, don't alias: assigning into os.environ here would leak the
        # checker's env into every later action and into this process itself.
        ...
        # A checker emitting non-UTF-8 bytes is a checker bug, but it should be
        # reported as such instead of crashing the validator with a traceback.
    ...
    def put(self, flag: str, flag_id: str, vuln: int):
        ...
        # ForcAD truncates with result.stdout[:1024] on the *bytes*, so measure
        # bytes here too: a non-ASCII flag id can fit in 1024 characters and
        # still be silently cut in half by the checker system.
        self._fatal(
            len(out.encode()) <= 1024, "returned stdout is longer than 1024 bytes"
        )
        self._fatal(
            len(err.encode()) <= 1024, "returned stderr is longer than 1024 bytes"
        )
    ...
    def run_all(self, step: int):
        ...
        for vuln in range(1, self._vulns + 1):
            ...
            # token_hex(20), not (16): ForcAD seeds private_flag_data with
            # secrets.token_hex(20), and a checker that stores the incoming
            # flag id in a fixed-width column should hit its limit here first.
            flag_id = self.put(flag=flag, flag_id=secrets.token_hex(20), vuln=vuln)
...
class StructureValidator(BaseValidator):
    ...
    def validate_networks(self, dc: dict, path: Path):
        """Refuse any declared docker network that overlaps the game (see
        GAME_NETWORK).

        Only explicit subnets are checked, which is the whole exposure: docker's
        own defaults live in 172.17+ and are clear of the game, so a collision
        has to be written down to happen.
        """
        ...
        for name, conf in networks.items():
            # `mynet:` with no body is a valid declaration meaning "defaults".
            ...
            for entry in ipam.get("config") or []:
                ...
                try:
                    ...
                    self._error(False, f"network {name} in {path} has an unparseable subnet {subnet!r}")
                    continue
                if net.version == GAME_NETWORK.version and net.overlaps(GAME_NETWORK):
                    self._error(
                        False,
                        f"network {name} in {path} uses {subnet}, which overlaps the "
                        f"game's {GAME_NETWORK}: this blackholes the team's own "
                        f"players from inside their vulnbox while the checker still passes",
                    )
    ...
    def validate_file(self, f: Path):
        ...
        if f.name == "docker-compose.yml":
            ...
            # The old ">=2.4, <3" rule existed because mem_limit/cpus/pids_limit
            # were only honoured by the v2 file format under docker-compose v1.
            # The Compose Spec used by the "docker compose" plugin honours them
            # regardless and ignores "version" entirely, printing a deprecation
            # warning when it is present.
            self._warning(
                "version" not in dc,
                f"version option in {path} is obsolete under the Compose Spec, remove it",
            )
            ...
            self.validate_networks(dc, path)
...
def dump_tasks(_args):
    ...
    for service in get_services():
        ...
        # ForcAD splits checker_type on "_" and only acts on the "pfr" and
        # "nfr" tags; "hackerdom" is its name for the plain flag-id-in-stdout
        # protocol, which is what checklib speaks.
        checker_type = "pfr" if info["attack_data"] else "hackerdom"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Got exception, report it:", e, tb, file=sys.stderr)
        exit(1)
